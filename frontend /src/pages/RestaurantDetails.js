...
import { FiHeart, FiMapPin, FiPhone, FiGlobe, FiClock, FiEdit2, FiTrash2, FiDollarSign, FiCheckCircle, FiFlag, FiExternalLink } from 'react-icons/fi';
...
export default function RestaurantDetails() {
  ...
  const [yelpReviewsLoading, setYelpReviewsLoading] = useState(false);
  const [yelpBusinessUrl, setYelpBusinessUrl] = useState('');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const fetchYelpReviewsForRestaurant = async (rest) => {
        if (!rest?.name || !rest?.city) {
          setYelpReviews([]);
          setYelpBusinessUrl('');
          return;
        }
        setYelpReviewsLoading(true);
        try {
          const location = [rest.city, rest.state].filter(Boolean).join(', ');
          const searchRes = await restaurantAPI.searchYelp({
            term: rest.name,
            city: location || rest.city,
            limit: 5,
          });
          const businesses = Array.isArray(searchRes?.data?.restaurants)
            ? searchRes.data.restaurants
          ...
          const exact = businesses.find(
            (b) =>
              normalize(b?.name) === normalize(rest.name) &&
              normalize(b?.city) === normalize(rest.city)
          ) || businesses[0];

          const yelpId = exact?.yelp_id || exact?.id;
          if (!yelpId) {
            setYelpReviews([]);
            setYelpBusinessUrl('');
            return;
          ...
          const yelpRevRes = await restaurantAPI.getYelpReviews(yelpId);
          setYelpReviews(yelpRevRes.data?.reviews || []);
          setYelpBusinessUrl(exact?.yelp_url || '');
        } catch {
          setYelpReviews([]);
          setYelpBusinessUrl('');
        } finally {
          setYelpReviewsLoading(false);
        }
      };
      ...
      try {
        ...
        setReplyDrafts(
          (revRes.data || []).reduce((acc, r) => {
            acc[r.id] = r.owner_reply || '';
            return acc;
          }, {})
        );
        fetchYelpReviewsForRestaurant(rest);
  ...
  const handleOwnerReply = async (reviewId) => {
    ...
    try {
      const res = await reviewAPI.replyToReview(reviewId, reply);
      const updated = res.data;
      setReviews((prev) => prev.map((r) => (r.id === reviewId ? updated : r)));
      setReplyDrafts((prev) => ({ ...prev, [reviewId]: updated.owner_reply || reply }));
    } catch (err) {
      console.error('Failed to save owner reply:', err);
    } finally {
  ...
  const canOwnerReply = user?.role === 'owner' && restaurant?.owner_id === user?.id;
  const canUserWriteReview = !!user && user.role !== 'owner';
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="space-y-4">
            {reviews.map((review) => (
              <div key={review.id} className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                ...
                {review.owner_reply && (
                  <div className="mt-3 ml-4 border-l-4 border-blue-200 bg-blue-50 rounded-r-lg px-4 py-3">
                    <p className="text-xs font-semibold text-blue-700 mb-1">
                      Owner reply{review.owner_reply_by ? ` • ${review.owner_reply_by}` : ''}
                    </p>
                    <p className="text-sm text-gray-700">{review.owner_reply}</p>
                  </div>
                )}
                {canOwnerReply && review.user_id !== user?.id && (
                  <div className="mt-4">
                    <label className="block text-xs font-semibold text-gray-600 mb-1">
                      {review.owner_reply ? 'Update reply' : 'Reply as owner'}
                    </label>
                    <textarea
                    ...
                    <div className="mt-2 flex justify-end">
                      <button
                        ...
                        disabled={replyingReviewId === review.id || !(replyDrafts[review.id] || '').trim()}
                        className="bg-yelp-red text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-yelp-dark disabled:opacity-50"
                      >
                        {replyingReviewId === review.id ? 'Saving...' : (review.owner_reply ? 'Update Reply' : 'Post Reply')}
                      </button>
                    </div>
                  </div>
                )}
      ...
      <div className="mt-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">Yelp Reviews</h2>
          {yelpBusinessUrl && (
            <a
              href={yelpBusinessUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="text-sm font-medium text-yelp-red hover:underline inline-flex items-center gap-1"
            >
              View on Yelp <FiExternalLink size={14} />
            </a>
          )}
        </div>
        {yelpReviewsLoading ? (
          <div className="bg-white rounded-xl shadow-sm p-8 text-center">
            <p className="text-gray-500">Loading Yelp reviews...</p>
          </div>
            ...
            {yelpReviews.map((review) => (
              <div key={review.id} className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center space-x-2 mb-1">
                      <div className="w-8 h-8 rounded-full bg-gray-200 overflow-hidden flex items-center justify-center text-sm font-bold text-gray-600">
                        {review.user?.image_url ? (
                          <img src={review.user.image_url} alt={review.user?.name || 'Reviewer'} className="w-full h-full object-cover" />
                        ) : (
                          <span>{review.user?.name?.charAt(0) || '?'}</span>
                        )}
                      </div>
                      <span className="font-medium text-gray-900">{review.user?.name || 'Yelp user'}</span>
                    </div>
                    <div className="flex items-center space-x-2 mb-2">
                      <StarDisplay rating={review.rating || 0} size={14} showNumber={false} />
                      <span className="text-xs text-gray-500">
                ...
                {review.text && <p className="text-gray-700 leading-relaxed">{review.text}</p>}
              </div>
            ))}
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow-sm p-8 text-center">
            <p className="text-gray-500">No Yelp reviews found for this restaurant match.</p>
          </div>
        )}
      </div>
    </div>
