...
def extract_intent(message: str) -> Dict[str, Optional[str]]:
    ...
    skip = {"i", "me", "my", "a", "an", "the", "for", "in", "at", "to", "of",
            ...
            "based", "preference", "preferences", "suggest", "according"}
...
def search_yelp_for_ai(
    ...
    term_parts = [p for p in [cuisine, ambiance, keyword] if p]
    ...
    for b in businesses:
        ...
        cat_titles = [c.get("title") for c in cats if c.get("title")]
        cuisine_type = ", ".join(cat_titles[:3]) if cat_titles else "Restaurant"
        cat_text = ", ".join(cat_titles).lower()
        inferred_ambiance = None
        if any(k in cat_text for k in ["romantic", "wine", "fine dining", "steakhouse", "bistro"]):
            inferred_ambiance = "romantic"
        elif any(k in cat_text for k in ["family", "kids"]):
            inferred_ambiance = "family-friendly"
        elif any(k in cat_text for k in ["casual", "cafe", "coffee"]):
            inferred_ambiance = "casual"
        ...
        out.append({
            ...
            "ambiance": inferred_ambiance,
            "dietary_options": None,
            "description": ", ".join(cat_titles) if cat_titles else "Live Yelp restaurant",
        })
    return out
...
def _split_pref_values(value: Optional[str]) -> List[str]:
    if not value:
        return []
    return [v.strip().lower() for v in value.split(",") if v.strip()]


def _contains_any(text: Optional[str], values: List[str]) -> bool:
    if not text or not values:
        return False
    t = text.lower()
    return any(v in t for v in values)


def _normalize_sort_preference(value: Optional[str]) -> str:
    v = (value or "").strip().lower()
    aliases = {
        "reviews": "popularity",
        "review": "popularity",
        "most_reviews": "popularity",
...
def _price_rank(price_value: Optional[str]) -> int:
    p = (price_value or "").strip()
    if not p:
        return 99
    return p.count("$") if "$" in p else 99


def _apply_preferences_to_intent(intent: Dict[str, Optional[str]], prefs: Optional[Dict[str, Any]]) -> Dict[str, Optional[str]]:
    """Fill missing intent fields from saved preferences so ranking is truly preference-driven."""
    merged = dict(intent)
    if not prefs:
        return merged

    # Treat generic meta keyword phrases as no keyword.
    kw = (merged.get("keyword") or "").strip().lower()
    generic_kw_tokens = {"preference", "preferences", "based", "suggest", "recommendation", "recommendations"}
    if kw:
        kw_words = [w for w in re.findall(r"\b\w+\b", kw) if w]
        if kw_words and all(w in generic_kw_tokens for w in kw_words):
            merged["keyword"] = None

    # If user did not provide a concrete cuisine, fallback to saved cuisine.
    # Do not block this fallback on generic keywords.
    if not merged.get("cuisine") and not merged.get("ambiance") and prefs.get("cuisine_preferences"):
        merged["cuisine"] = prefs["cuisine_preferences"].split(",")[0].strip()

    if not merged.get("dietary") and prefs.get("dietary_needs"):
        merged["dietary"] = prefs["dietary_needs"].split(",")[0].strip()

    if not merged.get("price_range") and prefs.get("price_range"):
        merged["price_range"] = prefs["price_range"]
...
def rank_recommendations(
    ...
    pref_dietary = _split_pref_values((prefs or {}).get("dietary_needs"))
    pref_cuisines = _split_pref_values((prefs or {}).get("cuisine_preferences"))
    romantic_terms = ["romantic", "wine", "fine dining", "bistro", "steakhouse", "candle", "date night", "$$$", "$$$$"]
    cuisine_targets = []
    if intent.get("cuisine"):
        cuisine_targets.append(intent["cuisine"].lower())
    cuisine_targets.extend(pref_cuisines)
    cuisine_targets = [c for c in cuisine_targets if c]

    def _haystack(r: Dict[str, Any]) -> str:
        return " ".join([
            str(r.get("cuisine_type") or ""),
            str(r.get("dietary_options") or ""),
            str(r.get("description") or ""),
            str(r.get("name") or ""),
        ]).lower()

    def _match_any(r: Dict[str, Any], targets: List[str]) -> bool:
        if not targets:
            return False
        h = _haystack(r)
        return any(t in h for t in targets)

    def _rkey(r: Dict[str, Any]) -> str:
        return f"{r.get('source','local')}|{r.get('yelp_id') or r.get('id')}|{(r.get('name') or '').strip().lower()}|{(r.get('city') or '').strip().lower()}"

    scored = []
    score_by_key: Dict[str, float] = {}
    for r in restaurants:
        score = score_restaurant(r, intent, prefs)
        haystack = _haystack(r)

        # Strongly prioritize explicit dietary preferences like vegan/vegetarian.
        if pref_dietary and _contains_any(haystack, pref_dietary):
            score += 45
        # If user asked for a dietary requirement explicitly in the query.
        if intent.get("dietary") and intent["dietary"].lower() in haystack:
            score += 30
        # Gentle boost for saved cuisine preference if present in text fields.
        if pref_cuisines and _contains_any(haystack, pref_cuisines):
            score += 15

        if intent.get("ambiance") == "romantic":
            if _contains_any(haystack, romantic_terms):
                score += 40
            else:
                # Lower rank for non-romantic places when user explicitly asks.
                score -= 20

            # Slight preference for mid-high price points for anniversary-style requests.
            if str(r.get("price_range") or "") in {"$$$", "$$$$"}:
                score += 12

        scored.append((score, r))
        score_by_key[_rkey(r)] = score

    scored.sort(key=lambda x: x[0], reverse=True)
    ranked = [r for _, r in scored]

    # Prefer rows that match dietary constraints; fallback to top-rated if none match.
    dietary_targets = []
    if intent.get("dietary"):
        dietary_targets.append(intent["dietary"].lower())
    dietary_targets.extend(pref_dietary)
    dietary_targets = [d for d in dietary_targets if d]

    if dietary_targets:
        strict = []
        for r in ranked:
            if _match_any(r, dietary_targets):
                strict.append(r)
        if strict:
            ranked = strict + [r for r in ranked if r not in strict]

    # If cuisine intent/preference is present, keep cuisine-matching restaurants first.
    # This prevents unrelated cuisine (e.g., Vietnamese) from outranking requested cuisine (e.g., Indian).
    if cuisine_targets:
        strict_cuisine = [r for r in ranked if _match_any(r, cuisine_targets)]
        if strict_cuisine:
            ranked = strict_cuisine + [r for r in ranked if r not in strict_cuisine]

    sort_pref = _normalize_sort_preference((prefs or {}).get("sort_preference"))
    def _base_score(r: Dict[str, Any]) -> float:
        return score_by_key.get(_rkey(r), 0.0)

    def _sort_bucket(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        if sort_pref == "price":
            return sorted(
                items,
                key=lambda r: (_price_rank(r.get("price_range")), -_base_score(r), -(r.get("average_rating") or 0), -(r.get("review_count") or 0)),
            )
        if sort_pref == "price_desc":
            return sorted(
                items,
                key=lambda r: (-_price_rank(r.get("price_range")), -_base_score(r), -(r.get("average_rating") or 0), -(r.get("review_count") or 0)),
            )
        if sort_pref == "rating":
            return sorted(
                items,
                key=lambda r: (-(r.get("average_rating") or 0), -(r.get("review_count") or 0), -_base_score(r), _price_rank(r.get("price_range"))),
            )
        if sort_pref == "popularity":
            return sorted(
                items,
                key=lambda r: (-(r.get("review_count") or 0), -(r.get("average_rating") or 0), -_base_score(r), _price_rank(r.get("price_range"))),
            )
        return sorted(items, key=lambda r: -_base_score(r))
    ...
    if cuisine_targets:
        matched = [r for r in ranked if _match_any(r, cuisine_targets)]
        unmatched = [r for r in ranked if not _match_any(r, cuisine_targets)]
        ranked = _sort_bucket(matched) + _sort_bucket(unmatched)
    else:
        ranked = _sort_bucket(ranked)
...
def chat_with_assistant(
    ...
    preferences = get_user_preferences(db, user_id) if user_id is not None else None
    intent_for_search = extract_intent(message)
    intent_for_search = _apply_preferences_to_intent(intent_for_search, preferences)
    ...
    ranked = rank_recommendations(all_restaurants, intent_for_search, preferences, limit=6)
    if ranked:
        message_text = build_conversational_message(intent_for_search, ranked, preferences, message)
        sort_pref = _normalize_sort_preference((preferences or {}).get("sort_preference"))
        if sort_pref == "price":
            message_text += " Sorted by lower price first based on your preferences."
        elif sort_pref == "popularity":
            message_text += " Sorted by review count (most reviewed first) based on your preferences."
        elif sort_pref == "rating":
            message_text += " Sorted by highest rating first based on your preferences."
        recommendations = [
            {
                "id": r.get("id"),
                "name": r.get("name"),
                ...
                "reason": build_reason(r, intent_for_search, preferences),
                "source": r.get("source", "local"),
                "yelp_id": r.get("yelp_id", ""),
            }
            for r in ranked
        ]
        return {"message": message_text, "recommendations": recommendations}
