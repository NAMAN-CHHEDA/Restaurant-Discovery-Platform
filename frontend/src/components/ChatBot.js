...
const GUEST_CHAT_STORAGE_KEY = 'chatbot_messages_guest_home';
...
export default function ChatBot() {
  ...
  const isGuestHome = !user?.id && location.pathname === '/';
  const activeStorageKey = user?.id ? getChatStorageKey(user.id) : (isGuestHome ? GUEST_CHAT_STORAGE_KEY : null);

  // Keep chat isolated per user/guest-home and reset otherwise.
  useEffect(() => {
    if (!activeStorageKey) {
      setIsOpen(false);
  ...
  if (!user && !isGuestHome) return null;
