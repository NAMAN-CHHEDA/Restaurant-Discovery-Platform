...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    ...
    ownerReviews.forEach((r) => {
      next[r.id] = r.owner_reply || '';
    });
    setOwnerReplyDrafts((prev) => ({ ...prev, ...next }));
  }, [ownerReviews]);
  ...
  const handleOwnerReplySubmit = async (reviewId) => {
    const reply = (ownerReplyDrafts[reviewId] || '').trim();
    if (!reply) return;
    setReplyingReviewId(reviewId);
    try {
      const res = await reviewAPI.replyToReview(reviewId, reply);
      const updated = res.data;
      setOwnerReviews((prev) => prev.map((r) => (r.id === reviewId ? updated : r)));
      setReviews((prev) => prev.map((r) => (r.id === reviewId ? updated : r)));
      setOwnerReplyDrafts((prev) => ({ ...prev, [reviewId]: updated.owner_reply || reply }));
    } catch (err) {
      // Keep dashboard stable even if one reply request fails.
    } finally {
            ...
            <div className="space-y-3">
              {ownerReviews.map((review) => (
                <div key={review.id} className="border border-gray-100 rounded-lg p-4">
                  ...
                  {review.owner_reply && (
                    <div className="mt-3 ml-2 border-l-4 border-blue-200 bg-blue-50 rounded-r-md px-3 py-2">
                      <p className="text-xs font-semibold text-blue-700">
                        Owner reply{review.owner_reply_by ? ` • ${review.owner_reply_by}` : ''}
                      </p>
                      <p className="text-sm text-gray-700 mt-1">{review.owner_reply}</p>
                    </div>
                  )}
                  <div className="mt-3">
                    <label className="block text-xs font-semibold text-gray-600 mb-1">
                      {review.owner_reply ? 'Update reply' : 'Reply to this review'}
                    </label>
                    <textarea
                      value={ownerReplyDrafts[review.id] || ''}
                      onChange={(e) => setOwnerReplyDrafts((prev) => ({ ...prev, [review.id]: e.target.value }))}
                      rows={2}
                    ...
                    <div className="mt-2 flex justify-end">
                      <button
                        ...
                        disabled={replyingReviewId === review.id || !(ownerReplyDrafts[review.id] || '').trim()}
