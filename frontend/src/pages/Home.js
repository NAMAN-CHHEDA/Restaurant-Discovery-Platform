...
const buildDedupeKey = (restaurant) => {
  ...
  const zip = normalizeText(restaurant?.zip_code);
  if (name && city && address) return `${name}|${city}|${address}`;
  if (name && city && zip) return `${name}|${city}|${zip}`;
  if (name && city) return `${name}|${city}`;
  return restaurant?.yelp_id ? `yelp:${restaurant.yelp_id}` : `id:${restaurant?.id}`;
};
...
export default function Home() {
  ...
  const fetchRestaurants = useCallback(async () => {
    ...
    // Deduplicate across BOTH sources (Yelp + local) using location identity.
    // This prevents showing the same place twice when a Yelp business is also
    // present in local DB (e.g., created for favorites/reviews/claim flow).
    const seen = new Set();
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {restaurants.map((r) => (
                    <RestaurantCard key={`${r.yelp_id || r.id}-${buildDedupeKey(r)}`} restaurant={r} />
                  ))}
                </div>
              </>
            ) : (
