...
export default function YelpRestaurantDetails() {
  ...
  const [localRestaurantMeta, setLocalRestaurantMeta] = useState(null);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setLocalRestaurantMeta(null);
      setClaimMsg('');
      try {
        const [detailsRes, reviewsRes] = await Promise.allSettled([
          restaurantAPI.getYelpById(yelpId),
          restaurantAPI.getYelpReviews(yelpId),
        ]);

        if (detailsRes.status === 'fulfilled') {
          setRestaurant(detailsRes.value.data);
        } else {
          setRestaurant(null);
        }

        if (reviewsRes.status === 'fulfilled') {
          setYelpReviews(reviewsRes.value.data?.reviews || []);
        } else {
          setYelpReviews([]);
        }
      } catch (e) {
  ...
  const ensureLocalRestaurantId = async () => {
    ...
    if (createdId) {
      ...
      setLocalRestaurantMeta(createRes.data || null);
      return createdId;
    }
    return null;
  };
  ...
  const handleClaimYelpRestaurant = async () => {
    if (!user) {
      navigate('/login');
      return;
    }
    ...
    try {
      const targetRestaurantId = await ensureLocalRestaurantId();
      if (!targetRestaurantId) throw new Error('Unable to create local restaurant record');
      const claimRes = await restaurantAPI.claim(targetRestaurantId);
      setLocalRestaurantMeta(claimRes.data || null);
      setClaimMsg('Restaurant claimed successfully.');
    } catch (err) {
      setClaimMsg(err?.response?.data?.detail || 'Failed to claim this restaurant.');
    } finally {
  ...
  const directionsUrl = addressLine
    ...
    : null;
  const ownerCanClaim = user?.role === 'owner';
  const canWriteReview = user?.role !== 'owner';
  const alreadyClaimedByMe = !!(localRestaurantMeta?.is_claimed && localRestaurantMeta?.owner_id === user?.id);
          ...
          <div className="pt-10 pb-8 h-[420px] flex flex-col justify-end">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              {ownerCanClaim && (
                <button
                  type="button"
                  onClick={handleClaimYelpRestaurant}
                  disabled={claimLoading || alreadyClaimedByMe}
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg font-semibold transition border ${
                    alreadyClaimedByMe
                ...
                >
                  {alreadyClaimedByMe ? <FiCheckCircle /> : <FiFlag />}
                  {alreadyClaimedByMe ? 'Claimed by you' : (claimLoading ? 'Claiming...' : 'Claim This Restaurant')}
                </button>
              )}
              {restaurant.yelp_url && (
                <a
            ...
            {claimMsg && (
              <p className={`mt-3 text-sm ${claimMsg.toLowerCase().includes('success') ? 'text-green-300' : 'text-red-300'}`}>
                {claimMsg}
              </p>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h2 className="text-lg font-bold text-gray-900 mb-4">Recent Yelp Reviews</h2>
              {reviewsLoading ? (
                <p className="text-sm text-gray-500">Loading Yelp reviews...</p>
              ) : yelpReviews.length === 0 ? (
                <p className="text-sm text-gray-500">No Yelp reviews available for this restaurant.</p>
              ) : (
                    ...
                    <div key={r.id} className="border border-gray-100 rounded-xl p-4">
                      <div className="flex items-center justify-between gap-3 mb-2">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-gray-200 overflow-hidden flex items-center justify-center">
                            {r.user?.image_url ? (
                              <img src={r.user.image_url} alt={r.user?.name || 'Reviewer'} className="w-full h-full object-cover" />
                            ) : (
                              <span className="text-xs font-semibold text-gray-600">{(r.user?.name || '?').charAt(0)}</span>
                            )}
                          </div>
                          <div>
                            <p className="text-sm font-semibold text-gray-900">{r.user?.name || 'Yelp user'}</p>
                            <p className="text-xs text-gray-500">{r.time_created ? new Date(r.time_created).toLocaleDateString() : ''}</p>
                          </div>
                        </div>
                        <StarDisplay rating={r.rating || 0} size={12} />
                      </div>
                      ...
                      {r.url && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 mt-2 text-sm text-yelp-red hover:underline"
                        >
                          Read on Yelp <FiExternalLink size={14} />
                        </a>
                      )}
                    </div>
                  ))}
